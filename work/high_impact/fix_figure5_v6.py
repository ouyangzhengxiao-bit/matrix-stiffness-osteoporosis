#!/usr/bin/env python3
"""Rebuild Figure 5 with panel-c statistics outside the plotting region."""
from pathlib import Path
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

ROOT = Path(__file__).resolve().parents[2]
V5 = ROOT / 'outputs/研究升级/high_impact_v5'
OUT = ROOT / 'outputs/研究升级/Communications_Biology_v6'
OUT.mkdir(parents=True, exist_ok=True)
t = pd.read_csv(V5 / 'temporal_transportability/chronological_transportability_results.csv')
d = pd.read_csv(V5 / 'multidonor_msc_atlas/donor_scores.csv')

plt.rcParams.update({
    'font.family': 'Arial', 'font.size': 8,
    'axes.spines.top': False, 'axes.spines.right': False,
    'pdf.fonttype': 42, 'ps.fonttype': 42,
})
fig, ax = plt.subplots(1, 3, figsize=(7.6, 2.75), gridspec_kw={'wspace': .54})

z = t[t['mode'] == 'fixed_earliest_three'].copy()
x = np.arange(len(z))
colors = ['#B24A4A' if v < 0 else '#3A78A8' for v in z.spearman_rho]
ax[0].bar(x, z.spearman_rho, color=colors, width=.68)
ax[0].axhline(0, color='black', lw=.7)
ax[0].set_xticks(x, z.held_out_study.str.replace('GSE', ''), rotation=40, ha='right')
ax[0].set_ylabel(r'Held-out Spearman $\rho$')
ax[0].set_xlabel('Later GEO study')
ax[0].set_title('Fixed earliest-three model', fontsize=9, pad=8)
ax[0].text(.02, .96, 'Median = −0.050', transform=ax[0].transAxes, va='top', fontsize=7)

ax[1].bar(x, z.top200_directional_agreement, color='#6B8E6B', width=.68)
ax[1].axhline(.5, color='black', lw=.8, ls='--')
ax[1].set_ylim(0, 1)
ax[1].set_xticks(x, z.held_out_study.str.replace('GSE', ''), rotation=40, ha='right')
ax[1].set_ylabel('Top-200 direction agreement')
ax[1].set_xlabel('Later GEO study')
ax[1].set_title('Chronological transfer', fontsize=9, pad=8)
ax[1].text(.02, .96, 'Median = 0.493', transform=ax[1].transAxes, va='top', fontsize=7)

order = ['Bone marrow', 'Adipose', 'Dermis', 'Umbilical cord']
palette = {'Bone marrow':'#C75B39', 'Adipose':'#D8A329',
           'Dermis':'#4C8F69', 'Umbilical cord':'#5678A6'}
for i, group in enumerate(order):
    vals = d.loc[d.tissue == group, 'absolute_top200_minus_matched_background'].to_numpy()
    xx = i + np.linspace(-.08, .08, len(vals))
    ax[2].scatter(xx, vals, s=25, color=palette[group], edgecolor='white', lw=.5, zorder=3)
    ax[2].plot([i-.18, i+.18], [vals.mean(), vals.mean()], color='black', lw=1.3)
ax[2].axhline(0, color='#777777', lw=.7)
ax[2].set_xticks(range(4), ['BM', 'Adipose', 'Dermis', 'UC'])
ax[2].set_ylabel('Matched expression-rank difference')
ax[2].set_title('Independent 11-donor atlas', fontsize=9, pad=28)
# Keep inferential text in a dedicated subtitle band above the axes so it cannot
# cover donor observations or their mean bars.
ax[2].text(.02, 1.015,
           'Bone vs others: exact $P$=0.218; four tissues: exact $P$=0.00299',
           transform=ax[2].transAxes, va='bottom', ha='left', fontsize=6.1,
           clip_on=False)

for i, axis in enumerate(ax):
    axis.text(-.2, 1.13, chr(97+i), transform=axis.transAxes,
              fontweight='bold', fontsize=11, clip_on=False)

fig.subplots_adjust(top=.82, bottom=.25, left=.08, right=.985)
fig.savefig(OUT/'Figure_5.png', dpi=600, bbox_inches='tight', pad_inches=.06)
fig.savefig(OUT/'Figure_5.tif', dpi=600, bbox_inches='tight', pad_inches=.06,
            pil_kwargs={'compression':'tiff_lzw'})
fig.savefig(OUT/'Figure_5.pdf', bbox_inches='tight', pad_inches=.06)
plt.close(fig)
print(OUT)
