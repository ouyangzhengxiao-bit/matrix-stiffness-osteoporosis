...
steps=[
 ...
 [sys.executable,'work/high_impact/run_multicohort_osteoporosis_validation.py'],
 [sys.executable,'work/high_impact/run_human_only_sensitivity.py'],
 ['Rscript','work/high_impact/make_v4_figures.R'],
 [sys.executable,'work/high_impact/make_figure4_multicohort.py']]
start_index=int(sys.argv[1]) if len(sys.argv)>1 else 0
log=[];start=time.time()
for cmd in steps[start_index:]:
 t=time.time();print('RUN',' '.join(cmd),flush=True);q=subprocess.run(cmd,cwd=ROOT)
 log.append({'command':cmd,'exit_code':q.returncode,'seconds':round(time.time()-t,2)})
 if q.returncode:raise SystemExit(q.returncode)
out=ROOT/'outputs/研究升级/high_impact_v4/full_pipeline_run_log_v50.json';out.write_text(json.dumps({'completed':True,'start_index':start_index,'total_seconds':round(time.time()-start,2),'steps':log},indent=2));print('WROTE',out)
